...
class Scoreboard(AppBase):
  ...
  def run(self):
    ...
    if self.data['games_played']:
      ...
      noGamesPlayed1 = "No games played"
      noGamesPlayed2 = "on " + str(self.data['date']['month']) + '/' + str(self.data['date']['day'])
      while True:
        offscreen_canvas.Clear()
        graphics.DrawText(offscreen_canvas, font, x_pos + 1, y_pos + 1, textColor, noGamesPlayed1)
        graphics.DrawText(offscreen_canvas, font, x_pos + 1, y_pos + 8, textColor, noGamesPlayed2)
        time.sleep(0.05)
        offscreen_canvas = self.matrix.SwapOnVSync(offscreen_canvas)
